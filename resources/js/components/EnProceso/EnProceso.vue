<template>
	<div>
        ...
        <SfDerivar :user="user" ref="derivarComponent" @derivado='isDerivado' ></SfDerivar>
        <SfArchivar  ref="archivarComponent" @archivado='isDerivado' ></SfArchivar>
	            ...
	            <div class="modal-content" v-if="documentoSelected">
                	...
                	<div class="modal-body scrollable" id="scrollStyle">
            			...
            			<div class="col-12 d-flex justify-content-between">
            				...
            				<!-- <a :href="'/seguimiento-externo/'+documentoSelected.documento.id" class="btn btn-sm btn-primary" target="printEvento2" onclick="window.open(this.href, this.target, ' width=800, height=700, menubar=no');return false;">
            					<i class="fas fa-file-invoice mr-2"></i>
            					<span class="d-none d-md-inline">Imprimir</span> Trámite
            				</a> -->
	    ...
	    <div class="modal fade" id="modalEditarDocumento" tabindex="-1" role="dialog">
	        <div class="modal-dialog  modal-600" role="document">
	            <div class="modal-content" v-if="documentoEditSelect">
	                <div class="modal-header">
	                ...
	                <form action="/editar-guardar" method="POST">
	                	...
	                	<div class="modal-body scrollable" id="scrollStyle">
	                		<div class="form-row mb-3">
							    ...
							    <div class="form-group col-sm-12 col-md-4">
							        ...
							        <input type="number" name="folios" id="nroDocumentoPersona" placeholder="00" v-model="folios" class="form-control">
							    </div>
							    ...
							    <div class="form-group col-sm-12">
							        ...
							        <textarea name="asunto" class="form-control" v-model="asunto"></textarea>
							    </div>
							    <div class="form-group col-sm-12">
							        <label>
							        	ADJUNTOS
							        </label>
							        <textarea name="adjuntos" class="form-control" v-model="adjuntos"></textarea>
							    </div>
							</div>
	            		</div>
	            		<div class="modal-footer">
							<button data-dismiss="modal" type="button" class="btn btn-sm btn-outline-danger">
		...
		<div class="card" >
			<div class="card-header  p-3">
				<h4 class="user-select-none m-0 p-0">
					<!-- <img src="/svg/en-proceso-tramite.svg" class="img-30 mr-2" alt=""> -->
					<i class="fad fa-cog fa-spin fa-lg fa-pulse mr-2"></i>
							...
							<template v-else>
								<tr v-for="(documento,index) in listDocumetos.data" :key="index" class="text-uppercase text-center">
									<td >
										<div class="btn-group">
		                                    <button type="button" 
		                                        ...
		                                        aria-expanded="false">
		                                        <i class="fas fa-cog mr-0 mr-md-1"></i>
		                                        <span class="d-none d-md-inline">Opciones</span> <i class="fas fa-sort-down ml-1 mr-0"></i>
		                                    </button>
		                                    <div class="dropdown-menu dropdown-menu-right dropdown-personal text-capitalize py-2">
		                                        <!-- <a href="#" class="dropdown-item  px-3 m-0" >
		                                            <img src="/svg/archivados-tramite.svg" alt="" class="img-20 mr-2"> 
		                                            <span>Archivar / Finalizar</span>
		                                        </a> -->
		                                        <a href="#" v-if="comprobarEditar(documento)" @click="openModalEdiar(documento)" class="dropdown-item  px-2 m-0">
		                                        	<i class="fas fa-edit text-primary mr-1"></i>
		                                        	<span>Editar</span>
		                                        </a>
		                                        <a href="#" @click="accionDerivar(documento)" class="dropdown-item  px-2 m-0" >
		                                        	<i class="fas fa-share-square text-success mr-1"></i>
		                                        	<span>Derivar</span>
		                                        </a>
		                                        <a :href="`/tramite/${documento.documento.id}/${documento.id}`" class="dropdown-item  px-2 m-0" >
		                                        	<i class="fas fa-share-all fa-flip-horizontal text-primary"></i>
		                                        	<span>Responder</span>
		                                        </a>
		                                        <a :href="`/tramite/${documento.documento.id}/${documento.id}/1`" class="dropdown-item  px-2 m-0" >
		                                        	<i class="fas fa-share-square text-primary fa-flip-horizontal mr-1"></i>
		                                        	<span>Responder y Adjuntar</span>
		                                        </a>
		                                        <a href="#" class="dropdown-item  px-2 m-0" @click="accionArchivar(documento)" >
		                                            <i class="fad fa-inbox-in mr-1 text-danger"></i>
		                                            <span>Archivar / Finalizar</span>
		                                        </a>
	                                        </div>
		                                </div>
									...
									<td class="text-center">
										<label class="label  text-uppercase" :class="{'label-success':user.id == documento.origen_usuario.id,'label-inverse':user.id != documento.origen_usuario.id}"><i class="fas fa-user fa-lg mr-2"></i> {{ documento.origen_usuario.username }}</label>
									</td>
									...
									<td>{{ documento.documento.origenDocumento == 0 ? "INTERNO":"EXTERNO" }}</td>
									<td class="text-left">
										<template  v-if="documento.documento.origenDocumento == 1" >
											<label v-if="documento.documento.tipoPersona == 1" class="label  label-warning">P. NATURAL</label>
											<label v-else class="label  label-inverse">P. JURIDICA</label>
										</template>
										<template  v-else >
											<label class="label label-primary"><i class="fas fa-university  mr-1"></i>MUNI SECCLLA</label>
										</template>
										{{ documento.documento.origenDocumento == 1 ? 
											(documento.documento.tipoPersona == 2 ? 
												documento.documento.dependencia : '') : 
												documento.documento.dependencia }}
										<span v-if="documento.documento.tipoPersona == 2 || documento.documento.origenDocumento == 0"><br > FIRMA: </span>
										<strong>{{ documento.documento.firma }}</strong>
									</td>
								</tr>
							</template>
...
<script src="@/components/EnProceso/EnProceso.js"></script>
<style scoped>
	.vue-html2pdf{
